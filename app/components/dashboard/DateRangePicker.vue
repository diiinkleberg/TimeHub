...
const currentRange = computed(() => {
  ...
  return mode.value === 'week'
    ? buildRange(
        startOfWeek(reference, 'de-DE'),
        endOfWeek(reference, 'de-DE')
      )
...
const shiftRange = (direction: 1 | -1) => {
  ...
  const nextStart
    = mode.value === 'week'
      ? startOfWeek(start.add({ weeks: direction }), 'de-DE')
      : startOfMonth(start.add({ months: direction }))

  const nextEnd
    = mode.value === 'week'
      ? endOfWeek(nextStart, 'de-DE')
...
const isNextDisabled = computed(() => {
  const referenceEnd
    = mode.value === 'week'
      ? endOfWeek(today(timeZone), 'de-DE')
